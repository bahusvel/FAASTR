...
impl Dmar {
    ...
    pub fn new(sdt: &'static Sdt) -> Option<Dmar> {
        if &sdt.signature == b"DMAR" && sdt.data_len() >= 12 {
            ...
            let rsv: [u8; 10] =
                unsafe { *((sdt.data_address() as *const u8).offset(2) as *const [u8; 10]) };
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.i + 4 <= self.sdt.data_len() {
            let entry_type = unsafe {
                *((self.sdt.data_address() as *const u8).offset(self.i as isize) as *const u16)
            };
            let entry_len = unsafe {
                *((self.sdt.data_address() as *const u8).offset(self.i as isize + 2) as *const u16)
            } as usize;

            if self.i + entry_len <= self.sdt.data_len() {
                let item = match entry_type {
                    0 => {
                        if entry_len >= mem::size_of::<DmarDrhd>() {
                            DmarEntry::Drhd(unsafe {
                                &*((self.sdt.data_address() + self.i) as *const DmarDrhd)
                            })
                        } else {
                    ...
                    1 => {
                        if entry_len >= mem::size_of::<DmarRmrr>() {
                            DmarEntry::Rmrr(unsafe {
                                &*((self.sdt.data_address() + self.i) as *const DmarRmrr)
                            })
                        } else {
                    ...
                    2 => {
                        if entry_len >= mem::size_of::<DmarAtsr>() {
                            DmarEntry::Atsr(unsafe {
                                &*((self.sdt.data_address() + self.i) as *const DmarAtsr)
                            })
                        } else {
                    ...
                    3 => {
                        if entry_len == mem::size_of::<DmarRhsa>() {
                            DmarEntry::Rhsa(unsafe {
                                &*((self.sdt.data_address() + self.i) as *const DmarRhsa)
                            })
                        } else {
                    ...
                    4 => {
                        if entry_len >= mem::size_of::<DmarAndd>() {
                            DmarEntry::Andd(unsafe {
                                &*((self.sdt.data_address() + self.i) as *const DmarAndd)
                            })
                        } else {
