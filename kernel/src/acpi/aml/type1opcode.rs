...
fn parse_def_break(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
fn parse_def_breakpoint(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
fn parse_def_continue(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
fn parse_def_noop(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
fn parse_def_fatal(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
fn parse_def_load(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
    ...
    if is_aml_table(sdt) {
        ...
        let _ = ctx.modify(
            ddb_handle_object.val,
            AmlValue::DDBHandle((delta, get_sdt_signature(sdt))),
        );
...
fn parse_def_notify(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
fn parse_def_release(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
fn parse_def_reset(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
fn parse_def_signal(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
fn parse_def_sleep(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
fn parse_def_stall(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
fn parse_def_unload(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
fn parse_def_if_else(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
    ...
    let (else_length, else_length_len) =
        if data.len() > 1 + pkg_length && data[1 + pkg_length] == 0xA1 {
    ...
    if if_condition.val.get_as_integer()? > 0 {
        parse_term_list(
            &data[1 + pkg_length_len + if_condition.len..1 + pkg_length],
            ctx,
        )?;
    } else if else_length > 0 {
        parse_term_list(
            &data[2 + pkg_length + else_length_len..2 + pkg_length + else_length],
            ctx,
        )?;
    }
...
fn parse_def_while(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
    ...
    loop {
        ...
        parse_term_list(
            &data[1 + pkg_length_len + predicate.len..1 + pkg_length],
            ctx,
        )?;
...
fn parse_def_return(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
