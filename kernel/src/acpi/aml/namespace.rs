...
impl AmlValue {
    ...
    pub fn get_as_buffer(&self) -> Result<Vec<u8>, AmlError> {
        match *self {
            ...
            AmlValue::String(ref s) => Ok(s.clone().into_bytes()),
    ...
    pub fn get_as_ddb_handle(&self) -> Result<(Vec<String>, SdtSignature), AmlError> {
        match *self {
            ...
            AmlValue::Integer(i) => {
                if let Some(sig) = get_signature_from_index(i as usize) {
                    Ok((vec![], sig))
                } else {
    ...
    pub fn get_as_integer(&self) -> Result<u64, AmlError> {
        match *self {
            ...
            AmlValue::DDBHandle(ref v) => {
                if let Some(idx) = get_index_from_signature(v.1.clone()) {
                    Ok(idx as u64)
                } else {
                    Err(AmlError::AmlValueError)
                }
