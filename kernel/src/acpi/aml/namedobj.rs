...
pub fn parse_named_obj(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
...
fn parse_def_bank_field(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    ...
    let bank_name = parse_name_string(
        &data[2 + pkg_length_len + region_name.len..2 + pkg_length],
        ctx,
    )?;
...
fn parse_def_create_bit_field(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    if data.len() < 2 {
        return Err(AmlError::AmlParseError(
            "DefCreateBitField - data truncated",
        ));
    }
    ...
    ctx.add_to_namespace(
        local_scope_string,
        AmlValue::BufferField(BufferField {
...
fn parse_def_create_byte_field(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    if data.len() < 2 {
        return Err(AmlError::AmlParseError(
            "DefCreateByteField - data truncated",
        ));
    }
...
fn parse_def_create_word_field(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    if data.len() < 2 {
        return Err(AmlError::AmlParseError(
            "DefCreateWordField - data truncated",
        ));
    }
...
fn parse_def_create_dword_field(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    if data.len() < 2 {
        return Err(AmlError::AmlParseError(
            "DefCreateDwordField - data truncated",
        ));
    }
...
fn parse_def_create_qword_field(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    if data.len() < 2 {
        return Err(AmlError::AmlParseError(
            "DefCreateQwordField - data truncated",
        ));
    }
...
fn parse_def_create_field(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    ...
    let name = parse_name_string(
        &data[2 + source_buf.len + bit_index.len + num_bits.len..],
        ctx,
    )?;
...
fn parse_def_data_region(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    ...
    ctx.add_to_namespace(
        local_scope_string,
        AmlValue::OperationRegion(OperationRegion {
...
fn parse_def_device(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    ...
    parse_object_list(
        &data[2 + pkg_length_len + name.len..2 + pkg_length],
        &mut local_ctx,
    )?;

    ctx.add_to_namespace(
        local_scope_string,
        AmlValue::Device(Device {
...
fn parse_def_index_field(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    ...
    let data_name = parse_name_string(
        &data[2 + pkg_length_len + idx_name.len..2 + pkg_length],
        ctx,
    )?;
...
fn parse_field_element(
    ...
    let length = if let Ok(field) = parse_named_field(data, ctx) {
        let local_scope_string =
            get_namespace_string(ctx.scope.clone(), AmlValue::String(field.val.name.clone()))?;

        ctx.add_to_namespace(
            local_scope_string,
            AmlValue::FieldUnit(FieldUnit {
...
fn parse_named_field(
    data: &[u8],
    _ctx: &mut AmlExecutionContext,
) -> Result<AmlParseTypeGeneric<NamedField>, AmlError> {
    if data.len() < 4 {
...
fn parse_reserved_field(
    data: &[u8],
    _ctx: &mut AmlExecutionContext,
) -> Result<AmlParseTypeGeneric<usize>, AmlError> {
    if data.len() < 1 {
...
fn parse_access_field(
    data: &[u8],
    _ctx: &mut AmlExecutionContext,
) -> Result<AmlParseTypeGeneric<AccessField>, AmlError> {
    if data.len() < 3 {
...
fn parse_def_power_res(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    ...
    let mut local_ctx = AmlExecutionContext::new(local_scope_string.clone());
    parse_object_list(
        &data[5 + pkg_len_len + name.len..2 + pkg_len],
        &mut local_ctx,
    )?;

    ctx.add_to_namespace(
        local_scope_string,
        AmlValue::PowerResource(PowerResource {
            system_level,
            resource_order,
            obj_list: local_ctx.namespace_delta.clone(),
        }),
...
fn parse_def_processor(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    ...
    let mut local_ctx = AmlExecutionContext::new(local_scope_string.clone());
    parse_object_list(
        &data[8 + pkg_len_len + name.len..2 + pkg_len],
        &mut local_ctx,
    )?;

    ctx.add_to_namespace(
        local_scope_string,
        AmlValue::Processor(Processor {
            proc_id: proc_id,
            p_blk: if p_blk_len > 0 {
                Some(p_blk_addr)
            } else {
                None
            },
...
fn parse_def_thermal_zone(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    ...
    let mut local_ctx = AmlExecutionContext::new(local_scope_string.clone());
    parse_object_list(
        &data[2 + pkg_len_len + name.len..2 + pkg_len],
        &mut local_ctx,
    )?;

    ctx.add_to_namespace(
        local_scope_string,
        AmlValue::ThermalZone(ThermalZone {
            obj_list: local_ctx.namespace_delta.clone(),
            notify_methods: BTreeMap::new(),
        }),
    )?;
