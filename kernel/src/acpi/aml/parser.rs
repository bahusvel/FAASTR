    ...
    fn modify_index_core(
        ...
        match obj {
            ...
            AmlValue::BufferField(ref b) => {
                ...
                let _ = self.modify(
                    AmlValue::ObjectReference(ObjectReference::Index(
                        b.source_buf.clone(),
                        Box::new(AmlValue::Integer(idx.clone())),
                    )),
                    value,
                );
    ...
    fn get_index_core(&self, obj: AmlValue, indices: Vec<u64>) -> Result<AmlValue, AmlError> {
        match obj {
            ...
            AmlValue::BufferField(ref b) => {
                ...
                Ok(AmlValue::Integer(
                    b.source_buf.get_as_buffer()?[idx as usize] as u64,
                ))
            }
            AmlValue::Package(ref p) => {
                if indices.len() == 0 {
