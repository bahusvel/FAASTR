...
pub fn parse_data_ref_obj(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
pub fn parse_arg_obj(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
pub fn parse_local_obj(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
fn parse_computational_data(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
    ...
    match data[0] {
        ...
        0x0B => {
            let res = (data[1] as u16) + ((data[2] as u16) << 8);
