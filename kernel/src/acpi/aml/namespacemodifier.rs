...
fn parse_alias_op(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
fn parse_name_op(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
fn parse_scope_op(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
