...
fn parse_multi_name_path(data: &[u8]) -> Result<(Vec<u8>, usize), AmlError> {
    ...
    if seg_count == 0x00 {
        return Err(AmlError::AmlParseError(
            "MultiName Path - can't have zero name segments",
        ));
    }
...
pub fn parse_super_name(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
fn parse_debug_obj(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
pub fn parse_simple_name(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
...
pub fn parse_target(data: &[u8], ctx: &mut AmlExecutionContext) -> ParseResult {
    match ctx.state {
        ExecutionState::EXECUTING => (),
        _ => {
            return Ok(AmlParseType {
