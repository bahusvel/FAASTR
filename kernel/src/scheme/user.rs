...
impl UserInner {
        ...
        if address == 0 {
            ...
            for i in 0..grants.len() {
                ...
                if address >= start && address < end {
                    grants.remove(i).unmap_inactive(
                        &mut new_table,
                        &mut temporary_page,
                    );
    ...
    pub fn read(&self, buf: &mut [u8]) -> Result<usize> {
        ...
        Ok(
            self.todo.receive_into(
                packet_buf,
                self.flags & O_NONBLOCK != O_NONBLOCK,
            ) * mem::size_of::<Packet>(),
        )
    }
    ...
    pub fn write(&self, buf: &[u8]) -> Result<usize> {
        ...
        while i < len {
            ...
            if packet.id == 0 {
                match packet.a {
                    SYS_FEVENT => {
                        event::trigger(self.scheme_id.load(Ordering::SeqCst), packet.b, packet.c)
                    }
                ...
                if let Some((context_weak, size)) = self.fmap.lock().remove(&packet.id) {
                    if let Ok(address) = Error::demux(packet.a) {
                        packet.a = Error::mux(
                            UserInner::capture_inner(&context_weak, address, size, true),
                        );
                    }
                }
    ...
    fn futimens(&self, file: usize, times: &[TimeSpec]) -> Result<usize> {
        ...
        let buf = unsafe {
            slice::from_raw_parts(
                ...
                mem::size_of::<TimeSpec>() * times.len(),
            )
        };
        let address = inner.capture(buf)?;
