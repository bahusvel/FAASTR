...
impl SchemeList {
    ...
    fn new_ns(&mut self) -> SchemeNamespace {
        ...
        self.insert(
            ns,
            Box::new(*b"env"),
            |_| Arc::new(Box::new(EnvScheme::new())),
        ).unwrap();
        self.insert(
            ns,
            Box::new(*b"memory"),
            |_| Arc::new(Box::new(MemoryScheme)),
        ).unwrap();
        self.insert(
            ns,
            Box::new(*b"sys"),
            |_| Arc::new(Box::new(SysScheme::new())),
        ).unwrap();
        self.insert(ns, Box::new(*b"time"), |scheme_id| {
            Arc::new(Box::new(TimeScheme::new(scheme_id)))
        }).unwrap();
    ...
    fn new_root(&mut self) {
        ...
        self.insert(ns, Box::new(*b"debug"), |scheme_id| {
            Arc::new(Box::new(DebugScheme::new(scheme_id)))
        }).unwrap();
        self.insert(ns, Box::new(*b"initfs"), |_| {
            Arc::new(Box::new(InitFsScheme::new()))
        }).unwrap();
        self.insert(ns, Box::new(*b"irq"), |scheme_id| {
            Arc::new(Box::new(IrqScheme::new(scheme_id)))
        }).unwrap();
        self.insert(ns, Box::new(*b"pipe"), |scheme_id| {
            Arc::new(Box::new(PipeScheme::new(scheme_id)))
        }).unwrap();
    }
    ...
    fn new_root(&mut self) {
        ...
        self.insert(ns, Box::new(*b"disk/live"), |_| {
            Arc::new(Box::new(self::live::DiskScheme::new()))
        }).unwrap();
        self.insert(ns, Box::new(*b"initfs"), |_| {
            Arc::new(Box::new(InitFsScheme::new()))
        }).unwrap();
    ...
    pub fn iter_name(&self, ns: SchemeNamespace) -> SchemeIter {
        SchemeIter { inner: self.names.get(&ns).map(|names| names.iter()) }
    }
    ...
    pub fn get_name(
        ...
        ns: SchemeNamespace,
        name: &[u8],
    ) -> Option<(SchemeId, &Arc<Box<Scheme + Send + Sync>>)> {
    ...
    where
        F: Fn(SchemeId) -> Arc<Box<Scheme + Send + Sync>>,
    {
        if let Some(names) = self.names.get(&ns) {
