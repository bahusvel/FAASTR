        ...
        match a & SYS_CLASS {
            SYS_CLASS_FILE => {
                ...
                match a & SYS_ARG {
                    ...
                    SYS_ARG_MSLICE => {
                        file_op_mut_slice(a, fd, validate_slice_mut(c as *mut u8, d)?)
                    }
                    _ => {
                        match a {
                            SYS_CLOSE => close(fd),
                            SYS_DUP => {
                                dup(fd, validate_slice(c as *const u8, d)?).map(FileHandle::into)
                            }
                            SYS_DUP2 => {
                                dup2(fd, FileHandle::from(c), validate_slice(d as *const u8, e)?)
                                    .map(FileHandle::into)
                            }
