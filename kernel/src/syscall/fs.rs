...
use syscall::error::*;
use syscall::flag::{F_GETFD, F_SETFD, F_GETFL, F_SETFL, F_DUPFD, O_ACCMODE, O_DIRECTORY, O_RDONLY,
                    O_WRONLY, MODE_DIR, MODE_FILE, O_CLOEXEC};
...
pub fn file_op(a: usize, fd: FileHandle, c: usize, d: usize) -> Result<usize> {
    ...
    let scheme = {
        ...
        let scheme = schemes.get(file.description.read().scheme).ok_or(
            Error::new(EBADF),
        )?;
        Arc::clone(&scheme)
    };
...
pub fn open(path: &[u8], flags: usize) -> Result<FileHandle> {
    ...
    let (scheme_id, file_id) = {
        ...
        let (scheme_id, scheme) = {
            let schemes = scheme::schemes();
            let (scheme_id, scheme) = schemes.get_name(scheme_ns, scheme_name).ok_or(
                Error::new(ENODEV),
            )?;
...
pub fn chmod(path: &[u8], mode: u16) -> Result<usize> {
    ...
    let scheme = {
        let schemes = scheme::schemes();
        let (_scheme_id, scheme) = schemes.get_name(scheme_ns, scheme_name).ok_or(
            Error::new(ENODEV),
        )?;
        Arc::clone(&scheme)
    };
...
pub fn dup2(fd: FileHandle, new_fd: FileHandle, buf: &[u8]) -> Result<FileHandle> {
    if fd == new_fd {
        ...
        context.insert_file(new_fd, new_file).ok_or(
            Error::new(EMFILE),
        )
    }
}
...
pub fn fcntl(fd: FileHandle, cmd: usize, arg: usize) -> Result<usize> {
    ...
    // Perform kernel operation if scheme agrees
    {
        ...
        match *files.get_mut(fd.into()).ok_or(Error::new(EBADF))? {
            Some(ref mut file) => {
                match cmd {
                    F_GETFD => if file.cloexec { Ok(O_CLOEXEC) } else { Ok(0) },
                    F_SETFD => {
