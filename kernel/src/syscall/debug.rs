...
pub fn format_call(a: usize, b: usize, c: usize, d: usize, e: usize, f: usize) -> String {
    match a {
        ...
        SYS_FEVENT => format!("fevent({}, {:#X})", b, c),
        SYS_FMAP => format!("fmap({}, {:#X}, {})", b, c, d),
        SYS_FUNMAP => format!("funmap({:#X})", b),
        ...
        SYS_GETCWD => format!("getcwd({:#X}, {})", b, c),
        ...
        SYS_MKNS => format!("mkns({:?})", validate_slice(b as *const [usize; 2], c)),
        SYS_NANOSLEEP => {
            format!(
                "nanosleep({:?}, ({}, {}))",
